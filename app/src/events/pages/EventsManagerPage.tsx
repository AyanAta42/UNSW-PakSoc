  ...
  return (
    <section className="mb-8">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-4">
        {events.map(ev => <AdminEventCard key={ev.id} event={ev} canEdit={canEdit} onAnnounce={onAnnounce} onUnpublish={onUnpublish} onEdit={onEdit} onDelete={onDelete} />)}
      </div>
    </section>
  )
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F3F4F6', fontFamily: 'system-ui,sans-serif' }}>
      <nav style={{ background: 'rgba(255,255,255,0.97)', borderBottom: '1px solid #E5E7EB' }} className="sticky top-0 z-50 h-14 flex items-center justify-between px-6 shadow-sm">
        ...
        {can.editEvents && <button onClick={() => setShowAdd(true)} style={{ background: G, color: '#fff' }} className="rounded-full px-5 py-1.5 font-bold text-sm cursor-pointer border-none hover:opacity-85 shadow-sm">+ New Event</button>}
      </nav>
      <div className="max-w-[1100px] mx-auto px-8 py-8">
        ...
        {!loading && <>
          {live.length > 0   && <EventSection title="Live on Home Page" color={G}       events={live}   {...sectionProps} />}
          {drafts.length > 0 && <EventSection title="Drafts"            color="#9CA3AF" events={drafts} {...sectionProps} />}
          {ended.length > 0  && <EventSection title="Ended"             color="#9CA3AF" events={ended}  {...sectionProps} />}
          {events.length === 0 && <div className="flex flex-col items-center justify-center py-28 text-center">
            <div className="text-base font-bold text-gray-400 mb-3">No events yet</div>
            {can.editEvents && <button onClick={() => setShowAdd(true)} style={{ background: G, color: '#fff' }} className="border-none rounded-full px-6 py-2.5 text-sm font-bold cursor-pointer hover:opacity-85 shadow-sm">+ Create your first event</button>}
          </div>}
        </>}
      </div>
