...
export function TaskListPanel({ tasks, loading, overTask, allCategories, eventId, mobile, currentUserAuthId, selectedMemberId, onRemoveTask, onRemoveAssigned, onEditTask, onAssignClick, onTaskClick }: Props) {
  ...
  return (
    <main className={`flex flex-col overflow-hidden min-w-0 bg-[#0B0E15] ${mobile ? 'flex-1 min-h-0 w-full' : 'hidden lg:flex flex-1'}`}>
      {!mobile && (
        <header className="h-14 px-7 bg-[#0B0E15] border-b border-[#1D2129] flex items-center gap-3 shrink-0">
          ...
          {currentUserAuthId && (
            <button onClick={() => setMyTasksOnly(v => !v)}
              className={`rounded-full px-4 py-1.5 text-xs font-bold border-2 cursor-pointer transition-all ${myTasksOnly ? 'bg-[#22C55E] text-white border-[#22C55E]' : 'bg-transparent border-[#2E333D] text-[#94A3B8] hover:border-[#22C55E] hover:text-[#4ADE80]'}`}>
              My Tasks
            </button>
          )}
          {hasSelected && (
      ...
      {mobile && currentUserAuthId && (
        <div className="px-4 py-2 border-b border-[#1D2129] shrink-0 bg-[#0B0E15]">
          <button onClick={() => setMyTasksOnly(v => !v)}
            className={`rounded-full px-4 py-1.5 text-xs font-bold border-2 cursor-pointer transition-all ${myTasksOnly ? 'bg-[#22C55E] text-white border-[#22C55E]' : 'bg-transparent border-[#2E333D] text-[#94A3B8] hover:border-[#22C55E]'}`}>
            My Tasks
          </button>
        </div>
      )}
                  ...
                  return (
                    <div key={task.id} data-task-id={task.id}
                      ...
                      <div className="absolute top-3 right-3 flex items-center gap-1">
                        <button onClick={e => { e.stopPropagation(); setEditingTask(task) }} aria-label="Edit task" className="text-[#94A3B8] hover:text-[#F8FAFC] bg-transparent border-none cursor-pointer text-[11px] font-semibold leading-none transition-colors px-1.5 py-1">Edit</button>
                        <button onClick={e => { e.stopPropagation(); setDeletingTaskId(task.id) }} aria-label="Delete task" className="text-[#64748B] hover:text-red-400 bg-transparent border-none cursor-pointer text-base leading-none transition-colors p-1">{'×'}</button>
                      </div>
