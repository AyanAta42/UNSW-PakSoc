...
export function RosterPanel({ members, loading, draggingMemberId, selectedMemberId, mobile, onBack, onMemberDragStart, onMemberDragMove, onMemberDragEnd, onMemberSelect }: Props) {
  ...
  return (
    <aside className={mobile
      ? 'flex flex-1 flex-col overflow-hidden min-h-0 w-full bg-[#0B0E15]'
      : 'hidden lg:flex w-[300px] shrink-0 flex-col overflow-hidden bg-[#0B0E15] border-r border-[#1D2129]'}>
      {!mobile && (
        <div className="p-5 border-b border-[#1D2129] shrink-0">
          <div className="flex items-center gap-2.5 mb-3">
            <button onClick={() => window.location.href = '/'} aria-label="Go to home"
              className="w-9 h-9 rounded-full border-2 border-[#1D2129] flex items-center justify-center text-[#94A3B8] cursor-pointer shrink-0 hover:border-[#22C55E] transition-all p-0 bg-transparent">
        ...
        {!loading && getMemberSections(members).map(section => (
          <div key={section.key} className="mb-1">
            <div className="flex items-center gap-2 px-4 lg:px-5 pt-3 pb-1.5">
              <span className="w-[3px] h-3.5 rounded-full shrink-0" style={{ background: section.color }} />
      ...
      {!mobile && <div className="px-5 py-3 border-t border-[#1D2129] text-[11px] text-[#64748B] shrink-0">One person can be assigned to many tasks.</div>}
    </aside>
  )
}
